...
from modDLC_vp import VideoProcessorCV as vp # used to CreateVideo
...
def create_labeled_video(config,videos,frame1,frame2,videotype='avi',shuffle=1,trainingsetindex=0,filtered=False,save_frames=False,Frames2plot=None,delete=False,displayedbodyparts='all',codec='mp4v',outputframerate=None, destfolder=None,draw_skeleton=False,trailpoints = 0,displaycropped=False):
    """
    ...
    for video in Videos:
        ...
        if os.path.isfile(os.path.join(str(videofolder),vname + DLCscorer+'_labeled.mp4')):
            ...
            Dataframe = Dataframe[frame1:frame2]
            ...
            if len(datanames)>0:
                ...
                if save_frames==True:
                    ...
                    if displaycropped: #then the cropped video + the labels is depicted
                        clip = vp(fname = video,sname = videooutname,codec=codec,sw=x2-x1,sh=y2-y1,i=frame1)
                        ...
                        clip = vp(fname = video,sname = videooutname,codec=codec,nframes=len(Dataframe),i=frame1)
                        CreateVideo(clip,Dataframe,cfg["pcutoff"],cfg["dotsize"],cfg["colormap"],DLCscorer,bodyparts,trailpoints,cropping,x1,x2,y1,y2,bodyparts2connect,skeleton_color,draw_skeleton,displaycropped)
